namespace Boilerplate.Web.Mvc.Sitemap
{
    ...
    public abstract class SitemapGenerator
    {
        ...
        private string GetSitemapDocument(IEnumerable<SitemapNode> sitemapNodes)
        {
            ...
            {
                var lastModifiedElement = sitemapNode.LastModified == null ? null : new XElement(
                    xmlns + "lastmod",
                    sitemapNode.LastModified.Value.ToLocalTime().ToString("yyyy-MM-ddTHH:mm:sszzz"));
                var frequencyElement = sitemapNode.Frequency == null ? null : new XElement(
                    xmlns + "changefreq",
                    sitemapNode.Frequency.Value.ToString().ToLowerInvariant());
                var priorityElement = sitemapNode.Priority == null ? null : new XElement(
                    xmlns + "priority",
